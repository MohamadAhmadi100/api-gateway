        ...
        if not settings.DEBUG_MODE:
            ...
            for number in settings.RECIPIENTS:
                ...
                if result.status_code == 200:
                    ...
                    print(
                        "الگوی مورد نظر پیدا نشد ، زمانی که نام الگو نادرست باشد یا طرح آن هنوز تائید نشده باشد رخ می‌دهد")
                elif result.status_code == 426:
                    print("استفاده از این متد نیازمند سرویس پیشرفته می‌باشد")
                elif result.status_code == 428:
                    print(
                        "ارسال کد از طریق تماس تلفنی امکان پذیر نیست، درصورتی که توکن فقط حاوی عدد نباشد این خطا رخ می‌دهد")
                elif result.status_code == 431:
                    print(
                        "ساختار کد صحیح نمی‌باشد ، اگر توکن حاوی خط جدید،فاصله، UnderLine یا جداکننده باشد این خطا رخ می‌دهد")
                elif result.status_code == 432:
                    print(
                        "پارامتر کد در متن پیام پیدا نشد ، اگر در هنگام تعریف الگو پارامتر token% را تعریف نکرده باشید این خطا رخ می‌دهد")
                else:
                    print("خطای نامشخص")
